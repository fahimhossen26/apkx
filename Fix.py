...
def init_db():
    ...
    # 2. User Profiles (Wallet & Referrals)
...
def add_balance(user_id, amount):
    """Adds (or subtracts) balance safely."""
    conn = get_db_connection()
    c = conn.cursor()
...
def process_referral(new_user_id, referrer_id):
    """Links referral and pays bonus."""
    if new_user_id == referrer_id: return False # Self referral
    ...
    if not c.fetchone():
        # Create referrer if missing (unlikely but safe)
        c.execute("INSERT INTO profiles (user_id) VALUES (?)", (referrer_id,))
    
    # Create new user with referrer link
...
def get_all_users():
    """Get list of all user IDs from profiles."""
    conn = get_db_connection()
    c = conn.cursor()
...
def replace_user_number(user_id):
    ...
    tag = current['tags']
    old_phone = current['phone'] # row factory uses column names
...
def delete_stock_db(country, tag, amount_str):
    ...
    if amount_str == 'ALL':
        ...
        # SQLite delete with limit workaround
    ...
    async with httpx.AsyncClient() as client:
        for source in API_SOURCES:
            params = {
                ...
                'records': 5,
                'dt1': dt1, # Crucial for filtering old messages
                'dt2': dt2
            }
            try:
                response = await client.get(source['url'], params=params, headers=headers, timeout=10.0)
                if response.status_code == 200:
                    try:
                        data = response.json()
                        if data.get('status') == 'success' and data.get('total', 0) > 0:
                            # Because we sent dt1, api filters time. 
                            # We grab the latest message from the result.
...
async def check_active_otps_job(context: ContextTypes.DEFAULT_TYPE):
    """
    Background job: Checks OTPs, Updates Message, Pays Rewards.
    Optimized for SQL.
    """
    conn = get_db_connection()
    c = conn.cursor()
    ...
    for row in active_assignments:
        ...
        if new_msg_text and new_msg_text != last_msg:
            ...
            if referrer_id:
                ...
                try:
                    await context.bot.send_message(chat_id=referrer_id, text=f"💰 **Commission Earned!**\nYour referral received an SMS.\nAdded: ${REWARD_COMMISSION}")
                except: pass

            try:
                await context.bot.send_message(chat_id=user_id, text=f"💰 **Reward:** You earned ${REWARD_PER_SMS} for receiving an SMS!")
            except: pass
...
# ------------------------------------------------------------------------
# ADMIN CONVERSATIONS (Delete / Broadcast / Upload)
# ------------------------------------------------------------------------
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    get_profile(user_id) # ensure exists
...
if __name__ == '__main__':
    ...
    broadcast_conv = ConversationHandler(
        ...
        states={ BROADCAST_MSG: [MessageHandler(filters.TEXT & ~filters.COMMAND, broadcast_perform)] },
        fallbacks=[CommandHandler('cancel', cancel_op)],
        per_message=False
    )
